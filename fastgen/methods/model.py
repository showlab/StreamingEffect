...
class FastGenModel(torch.nn.Module):
    ...
    def _prepare_training_data(self, data: Dict[str, Any]) -> tuple[torch.Tensor, Any, Any]:
        ...
        elif getattr(self.net, "is_v2v_bg", False):
            # handle V2V background replacement
            foreground_latent = data["foreground_latent"]
            ref_latents = data.get("ref_latents", None)
            ref_mask = data.get("ref_mask", None)
            ref_frame_indices = data.get("ref_frame_indices", None)

            # Ref sourcing priority:
            #   (1) random target-frame override if config.use_random_target_frame_as_ref
            #   (2) paired refs from data (any True in ref_mask)
            #   (3) GT tail frame if config.use_tail_frame_as_ref_fallback
            #   (4) leave as loaded (empty signal, all ref_mask False)
            # Network's _apply_ref_dropout still gates these with ref_dropout_prob.
            has_valid_refs = (
                torch.is_tensor(ref_mask) and bool(ref_mask.any().item())
            )
            use_random_target = getattr(
                self.config, "use_random_target_frame_as_ref", False
            )
            use_tail = getattr(self.config, "use_tail_frame_as_ref_fallback", False)
            if use_random_target:
                # Sample one random latent frame from GT (real_data) per step.
                # Network's collaborative-RoPE path uses a single index for the
                # whole batch (it indexes ref_frame_indices[0] internally), so a
                # per-step scalar matches the design.
                B, _, T, _, _ = real_data.shape
                rand_idx = int(torch.randint(0, T, (1,)).item())
                ref_latents = real_data[:, :, rand_idx:rand_idx + 1, :, :].clone()
                ref_mask = torch.ones(
                    B, 1, dtype=torch.float32, device=real_data.device
                )
                ref_frame_indices = [rand_idx]
            elif not has_valid_refs and use_tail:
                # real_data: [B, C, T, H, W] -> take last latent frame
                B, _, T, _, _ = real_data.shape
                ref_latents = real_data[:, :, -1:, :, :].clone()
                ref_mask = torch.ones(
                    B, 1, dtype=torch.float32, device=real_data.device
                )
                ref_frame_indices = [T - 1]

            condition = {
                "text_embeds": data["condition"],
                "foreground_latent": foreground_latent,
