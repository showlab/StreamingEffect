...
# =============================================================================
# Multi-Bucket Loader (multi-resolution training)
# =============================================================================


class MultiBucketWDSLoader:
    """Weighted sampling across multiple WDS buckets (different resolutions).

    Each bucket is a self-contained WDSLoader with its own datatags / key_map /
    files_map. On every ``next()`` we pick a bucket according to fixed weights
    (synchronized across ranks via a deterministic RNG that is seeded off
    ``seed`` — the seed is rank-independent, so every rank picks the same
    bucket at every step). A single batch is then drawn from that bucket's
    iterator. All items in a batch share the same resolution, so the default
    collate function works unmodified.

    Args:
        buckets: List of per-bucket dicts. Each dict must contain:
            - ``datatags``: List[str] in ``WDS:<path>`` format
            - ``weight``: float — sampling weight (will be normalized)
            Optional per-bucket overrides:
            - ``key_map``, ``files_map``, ``presets_map``, ``presets_filter``
            - ``shape``: List[int] (only used for logging/inspection)
        batch_size: Per-bucket batch size.
        sampler_start_idx: Forwarded to every sub-loader.
        seed: Rank-independent seed for bucket selection.
        num_workers: Workers per sub-loader. Keep small (default 1) since N
            buckets × num_workers dataloader processes are spawned per rank.
        shared_key_map / shared_files_map / shared_presets_map / shared_presets_filter:
            Defaults used when a bucket dict does not override them.
        **common_kwargs: Forwarded to every sub-loader (e.g. ``shuffle_size``,
            ``cache_path``, ``shard_count_file``, ``deterministic``, ``train``).
    """
    ...
    def __init__(
        ...
        shared_key_map: Optional[Dict[str, str]] = None,
        shared_files_map: Optional[Dict[str, str]] = None,
        shared_presets_map: Optional[Dict[str, str]] = None,
        shared_presets_filter: Optional[Dict[str, Dict[str, Any]]] = None,
        **common_kwargs: Any,
    ):
        ...
        if not buckets:
            raise ValueError("MultiBucketWDSLoader requires at least one bucket")

        self.loaders: List[WDSLoader] = []
        self.shapes: List[List[int]] = []
        weights: List[float] = []

        for i, b in enumerate(buckets):
            datatags = b.get("datatags")
            if not datatags:
                raise ValueError(f"bucket[{i}] missing required 'datatags'")

            kwargs: Dict[str, Any] = dict(common_kwargs)
            kwargs["datatags"] = datatags
            kwargs["batch_size"] = batch_size
            kwargs["sampler_start_idx"] = self.sampler_start_idx
            kwargs["num_workers"] = num_workers

            key_map = b.get("key_map", shared_key_map)
            if key_map is not None:
                kwargs["key_map"] = dict(key_map)

            files_map = b.get("files_map", shared_files_map)
            if files_map is not None:
                kwargs["files_map"] = dict(files_map)

            presets_map = b.get("presets_map", shared_presets_map)
            if presets_map is not None:
                kwargs["presets_map"] = dict(presets_map)

            presets_filter = b.get("presets_filter", shared_presets_filter)
            if presets_filter is not None:
                kwargs["presets_filter"] = dict(presets_filter)

            logger.info(f"[MultiBucket] initializing bucket[{i}] datatags={datatags}")
            loader = WDSLoader(**kwargs)
            self.loaders.append(loader)
            self.shapes.append(list(b.get("shape", [])))
        ...
        lines = [
            f"  [{i}] shape={self.shapes[i]} prob={self.probs[i]:.4f}"
            for i in range(len(self.loaders))
        ]
        logger.info("MultiBucketWDSLoader initialized with buckets:\n" + "\n".join(lines))
