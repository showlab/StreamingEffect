...
def init_wandb(config: BaseConfig):
    ...
    # Force-update config on every init (including resumes) so each run segment
    # has the exact config that was used, not just the config from the first init.
    wandb.config.update(config_resolved, allow_val_change=True)

    # Save config.yaml locally in the wandb run directory for easy comparison
    if wandb.run and wandb.run.dir:
        import yaml
        config_path = os.path.join(wandb.run.dir, "config.yaml")
