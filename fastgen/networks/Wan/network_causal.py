    ...
    for idx, block in enumerate(self.blocks):
        ...
        if hasattr(self, "_external_self_kv_list") and idx < len(self._external_self_kv_list):
            ...
            if cache_tag in original_cache:
                ...
                # For V2VBG: prefix tokens (ref+cond) are stored before target tokens
                # in the KV cache, so we add the prefix offset for chunks after the first.
                cache_start_tokens = attention_kwargs.get("cache_start_tokens", None)
                if cache_start_tokens is not None:
                    proper_cache_len = cache_start_tokens
                else:
                    prefix_offset = attention_kwargs.get("cache_prefix_offset", 0) if cur_start_frame > 0 else 0
                    proper_cache_len = prefix_offset + cur_start_frame * frame_seqlen
